...
### `engine_sound_volume`
- **書式**: 浮動小数点数(既定値: エンティティ種別により`1.0`または`3.0`。下記参照)
- **説明**: `engine_sound`の音量倍率。MCヘリ武器設定の`SoundVolume`と同じ考え方で、
  `1.0`が通常の最大音量、それを超えると聞こえる距離が伸びます(値に比例して
  全音量到達距離・無音になる距離の両方が伸びます)。
- **既定値について**: 明示的に指定しない場合、`entity_type`に応じて以下の
  既定値が使われます。
  - `aircraft`・`helicopter`・`vtol`(高高度を飛行するため)、および
    `ship`・`submarine`(艦船としての規模のため): `3.0`
  - `car`・`static_emplacement`、およびアドオンMODが追加する`entity_type`:
    `1.0`
- **例**:
  ```json
...
- **説明**: MCヘリの`FuelSupplyRange`/`AmmoSupplyRange`、および本プロジェクト
  ...
  補給は停止中の機体のみが対象です。弾薬は弾倉を補充した後、予備弾薬を
  武器の`MaxAmmo`(弾倉を含む総容量)まで補充します。
- **例**:
  ```json
